...
def prepareData(trainX, trainY, testX, testY):
    ...
    # Normalize the pixel values of the train and test datasets to be between 0 and 1
    trainX = trainX / 255.0
    testX = testX / 255.0

    #TODO - Application 1 - Step 4c - Transform the classes labels into a binary matrix
    # Print the shape of the training data
    print("Training data shape (images):", trainX.shape)
    print("Training labels shape:", trainY.shape)

    # Print the shape of the test data
    print("Test data shape (images):", testX.shape)
    print("Test labels shape:", testY.shape)


    return trainX, trainY, testX, testY
# Exercise 1: Display the first 9 images from the trainX dataset
(trainX, trainY), (testX, testY) = fashion_mnist.load_data()
...
for i in range(9):
    ...
    cv2.putText(img_resized, class_names[label], (10, 250), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
